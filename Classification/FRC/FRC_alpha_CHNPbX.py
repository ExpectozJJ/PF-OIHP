...
def func(idx):
    ...
    for ii in range(ndata):
        ...
        # file = open(fname)
        file = open('./'+folder+'/CHNPbX_atmlist_L5_f{}.txt'.format(idx))
    ...
    np.savez('./'+folder+"/CHNPbX_atmlist_L5_f{}_frcstat.npz".format(idx),  fdata = fdata)

#arr = np.concatenate((range(603, 608), range(625, 633), range(666, 677), range(728, 731), range(780, 794))) # Cl3 Cubic CHNPbX
#arr = np.concatenate((range(828, 835), range(860, 865), range(921, 922), range(956, 960), range(993, 996)))
arr = [889]
for val in arr:
    func(val)
...
for idx in range(begin, last):
    ...
    for ii in range(ndata):
        ...
        #print(contents)
        out = []
        for f in np.arange(0, fmax, step):
            sc = GeneralisedFormanRicci(points=contents, epsilon=f, method="alpha", p=2)
            frc = sc.compute_forman()
            features = []
            for key in range(3):
        ...
        statdata[ii] = out
        # print(fno)
    fdata = np.reshape(statdata, (ndata, fid*3*10))
    np.savez('./'+folder+"/CHNPbX_atmlist_L5_f{}_frcstat.npz".format(idx),  fdata = fdata) 
end = time.time()        
